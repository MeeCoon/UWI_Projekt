...
function yearsKey(companyId) {
  return `uwi_years_${companyId}_balance`;
}
...
function computeSaldo(rows) {
  ...
  for (const r of rows) {
    ...
    saldo[debit] = (saldo[debit] || 0) + amt;   // Soll +
    saldo[credit] = (saldo[credit] || 0) - amt; // Haben -
  }
  return saldo;
}
...
function applyBalance(companyId, year) {
  ...
  // Jahr global merken (für Buchungen)
  localStorage.setItem("uwi_currentBalanceYear", year);
  ...
  document.getElementById("totalAktiven")
    ?.replaceChildren(document.createTextNode(fmtCHF(totalAkt)));

  document.getElementById("totalPassiven")
    ?.replaceChildren(document.createTextNode(fmtCHF(totalPas)));
