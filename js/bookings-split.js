...
function createLine(side){
 ...
 s.className="balanceInput";
 s.innerHTML=buildAccountOptions();

 const i=document.createElement("input");
 i.type="number";
 i.min="0";
 i.placeholder="Betrag";
 i.className="balanceInput";
 ...
 s.onchange=updateSums;
 i.oninput=updateSums;
...
document.addEventListener("DOMContentLoaded",()=>{
 ...
 const cid=localStorage.getItem(currentCompanyKey(user));
 ...
 document.getElementById("logoutBtn")?.addEventListener("click",()=>{
  ...
  if(u)
   localStorage.removeItem(`uwi_currentCompany_${u}`);
